...
import { getMyTeams, createTeam, updateTeam, deleteTeam } from '../services/teamService';
...
const emptyTeam = () => Array(6).fill(null).map(emptySlot);

const TeamBuilder = () => {
  const { isAuthenticated } = useAuth();
  const [team, setTeam] = useState(emptyTeam());

  // Persistencia de equipos (asociados al usuario)
  ...
  const refreshTeams = async () => {
    ...
    try {
      ...
      showFeedback('error', 'No se pudieron cargar tus equipos');
    } finally {
  ...
  useEffect(() => {
    ...
    else { setMyTeams([]); setCurrentTeamId(null); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAuthenticated]);

  const handleSaveTeam = async () => {
    const name = teamName.trim() || 'Equipo sin nombre';
    setSaving(true);
    try {
      if (currentTeamId) {
        const updated = await updateTeam(currentTeamId, { name, slots: team });
        showFeedback('ok', `Equipo "${updated.name}" actualizado`);
      } else {
        const created = await createTeam({ name, slots: team });
        setCurrentTeamId(created._id);
        showFeedback('ok', `Equipo "${created.name}" guardado`);
      }
      refreshTeams();
    } catch {
      showFeedback('error', 'Error al guardar el equipo');
    } finally {
      setSaving(false);
    }
  };

  const handleLoadTeam = (t) => {
    // Rellenamos hasta 6 ranuras por seguridad
    const slots = Array.isArray(t.slots) ? t.slots.map(s => s || emptySlot()) : [];
    while (slots.length < 6) slots.push(emptySlot());
    setTeam(slots.slice(0, 6));
    setTeamName(t.name);
    setCurrentTeamId(t._id);
    setShowTeamsPanel(false);
    showFeedback('ok', `Equipo "${t.name}" cargado`);
  };

  const handleDeleteTeam = async (e, id) => {
    e.stopPropagation();
    if (!window.confirm('¿Eliminar este equipo de forma permanente?')) return;
    try {
      await deleteTeam(id);
      if (currentTeamId === id) setCurrentTeamId(null);
      refreshTeams();
      showFeedback('ok', 'Equipo eliminado');
    } catch {
      showFeedback('error', 'No se pudo eliminar');
    }
  };
  ...
  return (
    <div className="container tb-page" onClick={() => { setActiveMoveDropdown(null); setActiveItemDropdown(null); setShowTeamsPanel(false); }}>
      <div className="team-builder-header">
        <h1 className="page-title">Team Builder</h1>
        <div className="team-actions">
          <button className="page-btn clear-btn" onClick={() => { setTeam(emptyTeam()); }}>
            🗑️ Limpiar Todo
          </button>
        </div>
      </div>

      {/* 💾 BARRA DE GUARDADO DE EQUIPOS */}
      <div className="team-save-bar" onClick={(e) => e.stopPropagation()}>
        {isAuthenticated ? (
          <>
            <input
              type="text"
            ...
            />
            <button className="page-btn save-btn" onClick={handleSaveTeam} disabled={saving}>
              {saving ? 'Guardando...' : (currentTeamId ? '💾 Actualizar' : '💾 Guardar equipo')}
            </button>
            <button className="page-btn" onClick={() => { setShowTeamsPanel(s => !s); if (!showTeamsPanel) refreshTeams(); }}>
              📂 Mis equipos {myTeams.length > 0 && `(${myTeams.length})`}
            </button>
            {currentTeamId && (
              <button className="page-btn" onClick={handleNewTeam}>✨ Nuevo</button>
            )}

            {showTeamsPanel && (
              <div className="teams-panel">
                <div className="teams-panel-header">Mis equipos guardados</div>
                {loadingTeams ? (
                  <Loader text="Cargando..." size={40} />
                ) : myTeams.length === 0 ? (
                  <p className="teams-empty">Aún no has guardado ningún equipo.</p>
                ) : (
                  myTeams.map((t) => (
                    <div key={t._id} className={`team-panel-row ${currentTeamId === t._id ? 'active' : ''}`} onClick={() => handleLoadTeam(t)}>
                      <div className="team-panel-info">
                        <span className="team-panel-name">{t.name}</span>
                        <span className="team-panel-count">
                          {(t.slots || []).filter(s => s && s.pokemon).length} Pokémon
                        </span>
                      </div>
                      <button className="team-panel-delete" onClick={(e) => handleDeleteTeam(e, t._id)}>🗑️</button>
                    </div>
                  ))
                )}
              </div>
          ...
          <div className="team-login-hint">
            🔒 <Link to="/login" className="team-login-link">Inicia sesión</Link> para guardar tus equipos y acceder a ellos desde cualquier dispositivo.
          </div>
        )}

        {feedback && (
          <div className={`team-feedback ${feedback.type}`}>{feedback.text}</div>
        )}
      </div>
